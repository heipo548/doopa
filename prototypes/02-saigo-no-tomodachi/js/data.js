...
const BALANCE = {
  ...
  kokoroRegenPerTurn: 1,     // 毎プレイヤーターン、こころが自然に戻る量（息をつく＝ハードロック防止）
                             // ※2→1：毎ターン+2は戻りが速く「ぶつける乱用が実質無料」に見えた（実機FB「回復早すぎ」）。
                             //   1なら ウェーブ内で“重い一撃で仕留める/軽い手で凌ぐ”の天秤が効く。probe実測でも
                             //   harsh/gentle とも完走（むしろ harsh の完走率は1の方が高い）＝祓い道は詰まない。
  kokoroRegenDefendBonus: 2, // 「こらえる」ターンは追加で戻る（毎ターン+1の中で+2は効く＝こらえるの価値が上がる）
...
const KIND_WORDS = {
  ...
  // ── 世界が広がることば（v0.12・Toby主導の世界拡張）＝覚えると“見え方/できること”が変わる生活語 ──
  //   効果は控えめ（既存レンジ内）でバランス不変。役割は「世界を解凍する」こと。
  mata:        { name: "また ね",          word: "また ね",          category: "yasashii", heal: 2,    nukumori: 1, desc: "やさしい：別れぎわの ひとこと。HP＋2 ／ やさしい＋" },
  itterasshai: { name: "いってらっしゃい",  word: "いってらっしゃい",  category: "yasashii", atkDown: 1, nukumori: 1, desc: "くらしのことば：送り出す。相手が おちつく ／ やさしい＋" },
  oboeteru:    { name: "おぼえてるよ",      word: "おぼえてるよ",      category: "yasashii",             nukumori: 2, desc: "やさしい：わすれない、という約束 ／ やさしい＋＋" },
  // ……みずおと：効果は ほぼ無い。でも 言うと、みんなが すこし 耳を すます（ログだけ・lull＝戦闘演出フラグ）。
  mizuoto:     { name: "……（みずおと）",   word: "……",              category: "silence",  lull: true,  nukumori: 0, desc: "沈黙：みずの おと。なにも 起きない。でも、みんな きいている" },
};
...
const ENEMIES = {
  ...
  nushi: { name: "ぬしさま",     hp: 60, atk: 5, target: "all",    wall: 6, acts: ["uta", "nade", "negau"],
           ...
           // こころのかべが 半分ほど ほどけた最初のターンに ぽつりと（説得が弾＝攻撃の重さに効く）。
           phaseLine: "……もう、いいのかな。",
           // 2周目以降（meta._journeys>=1）は、ぬしさまが きみを 覚えている（周回のメタ・不穏）。
           phaseLineSeen: "……きみ、まえにも 来たね。" },
  ...
  // ── 場所に紐づく 道だけの友（v0.12 世界拡張）。WAVES には出ない＝戦闘バランス不変。場所＝place で対応。──
  mizu:  { name: "みずのこ",     hp: 5,  atk: 1, target: "single", wall: 1, acts: ["nade"], place: "mizuumi",
           shape: "ghost",  color: "#6ec3d8", sad: true,  gift: "mata",
           hint: "みなもを じっと のぞいている",
           flavor: "みずに うつった じぶんと、ずっと はなしてた子。" },
  eki:   { name: "えきのこ",     hp: 6,  atk: 1, target: "single", wall: 1, acts: ["nade"], place: "eki",
           shape: "circle", color: "#c9b48a", sad: true,  gift: "itterasshai",
           hint: "こない でんしゃを まっている",
           flavor: "だれかを ずっと まってる。でんしゃは、もう こない。" },
  oku:   { name: "おくのこ",     hp: 6,  atk: 1, target: "single", wall: 1, acts: ["nade"], place: "mori",
           shape: "bunny",  color: "#8f7fc0", sad: true,  gift: "oboeteru",
           hint: "なまえを おもいだそうと している",
           flavor: "もりの いちばん おくで、なまえを わすれた子。" },
  // 影＝さいごの夜「群れの おく」だけに うずくまる。救えるが gift なし・無言で消える（第三の名残）。
  //   FIELD_FRIEND_POOL には入れない（通常の道には出ない）。
  kage:  { name: "なまえのない 影", hp: 4, atk: 0, target: "single", wall: 1, acts: ["nade"],
           shape: "circle", color: "#3a3658", sad: true,
           hint: "だれだか、おもいだせない",
           flavor: "だれだか、おもいだせない。でも、しってる きがする。" },
};

// 敵の“声”＝きいてみる が効いた相手が ぽつりと こぼす一言（戦闘に「対話してる感」を出す）。
//   効いた時：その子の voice を 吹き出しで返す／効かない時は別の戸惑い（ui 側で汎用）。
// 敵の声は2段：closed＝こころのかべが まだ残るうちの“はねのける/警戒/まだ信じてない”声、
//   opening＝かべが ほどけた瞬間の やさしい声。＝救いは「正しい手順で開く宝箱」でなく「根気で ほどける」もの。
//   ※旧形式（ただの配列）も opening 扱いでフォールバック可（battle.js pickEnemyVoice）。第一夜くろまるは TUTORIAL 側。
const ENEMY_VOICE = {
  kuro:  { closed: ["……ちかよらないで。", "なに。ぼくに かまうな。", "どうせ、また いなくなる。"],
           opening:["……き、きいて くれるの？", "ことば、ひさしぶり。", "ひとりじゃ、なかった？"] },
  fura:  { closed: ["ふらふら……（目を そらす）", "ここに いちゃ、だめ？", "……みないで。"],
           opening:["ここ、いて いいの……？", "なまえ、よんで くれた。", "ふわ……すこし、とまれた。"] },
  usagi: { closed: ["ひっく……（泣いて きこえてない）", "ほっといて よ……。", "どうせ わからない。"],
           opening:["ぐすっ……うん。", "こわかったんだ、ずっと。", "なみだ、とまるかな。"] },
  toge:  { closed: ["とげ、ひっこめない。", "……さわったら、いたいよ。", "まだ、しんじてない。"],
           opening:["とげ、ひっこめて みる……。", "ほんとは、こわいだけ。", "さわっても、いい……？"] },
  nushi: { closed: ["……ことばは、もう いらない。", "なにを いっても、おなじ。", "きみも、どうせ。"],
           // opening に“問い返し”を一つ＝答えを言わず プレイヤーに投げ返す（夜が長い理由は明かさない）。
           opening:["……ひさしぶりに、声を きいた。", "みんなの ねがいが、きこえる？", "きみ、なら……。", "……よあけを、だれが わすれたと おもう？"] },
  hoshi: { closed: ["……まぶしいの、にがて。", "ひとりが、いい。"],
           opening:["ぴかっ……（うれしそう）", "おちてきて、よかった。"] },
  mori:  { closed: ["はっぱの かげに かくれる。", "……だれ？"],
           opening:["はっぱ、わけて あげる。", "もり、いっしょに かえろ？"] },
  mizu:  { closed: ["みなもが、ゆれる。", "……みないで。じぶんが きえる。"],
           opening:["きみも、みえるの？ この みず。", "ふたりなら、うつっても いい。"] },
  eki:   { closed: ["でんしゃ、まだ こない。", "……きみ、ちがう。"],
           opening:["まつの、もう やめても いい？", "いっしょに、いこうか。"] },
  oku:   { closed: ["……なまえ、なんだっけ。", "よばないで。わすれちゃう。"],
           opening:["きみの こえ、おぼえてる きがする。", "ぼくの なまえ、よんで くれる？"] },
  kage:  { closed: ["……ぽち？"] }, // opening 無し＝救うと無言で消える（第三の名残）
};

// 倒した子の「空席」を 街で さわったときの 名残＝その子の flavor を“途中で切った”文（説明しきらない）。
//   救いには顔があるのに 喪失には顔がない、を埋める。数（kill数）は出さない（STORY §3「不在を残す」）。
const TOWN_GHOST_LINES = {
  kuro:  "夜にこぼれた ちいさな影。ほんとは——",
  fura:  "ふわふわ さまよっていた。名前を 呼べば、すこし——",
  usagi: "ずっと 泣いていた子。やさしい ことばが、あれば——",
  toge:  "とげで 身を まもっていた。ほんとは、ことばが——",
  nushi: "群れの ねがいを 束ねていた。ことばに できたのは、たぶん——",
  hoshi: "おちてきた ほしの かけら みたいな子。くらいほうが——",
  mori:  "夜の もりから まよいでた子。すこし さみしそうで——",
  mizu:  "みずに うつった じぶんと はなしていた子。ほんとは——",
  eki:   "こない でんしゃを まっていた子。あの子は きっと——",
  oku:   "なまえを わすれた子。よんで あげれば、きっと——",
  kage:  "なまえが、なかった子。それとも——",
};

// 「おぼえてるよ」を覚えた者だけ、名残を“最後まで”見届けられる（#5）＝途中で切れない完結文。
const TOWN_GHOST_FULL = {
  kuro:  "夜にこぼれた ちいさな影。ほんとは、ことばを かわしたいだけ だった。",
  fura:  "ふわふわ さまよっていた。名前を 呼べば、すこし 止まれた 子だった。",
  usagi: "ずっと 泣いていた子。やさしい ことばが、なみだを とめられた かもしれない。",
  toge:  "とげで 身を まもっていた。ほんとは、ことばが こわかった だけ だった。",
  nushi: "群れの ねがいを 束ねていた。その願いを、きみは きいて あげられた かもしれない。",
  hoshi: "おちてきた ほしの かけら みたいな子。くらいほうが、まぶしくなくて よかった子。",
  mori:  "夜の もりから まよいでた子。ほんとは、いっしょに かえりたかった。",
  mizu:  "みずに うつった じぶんと はなしていた子。ほんとは、ひとりが さみしかった。",
  eki:   "こない でんしゃを まっていた子。あの子は きっと、まだ まっている。",
  oku:   "なまえを わすれた子。よんで あげれば、きっと おもいだせた。",
  kage:  "なまえが、なかった子。それとも、きみの わすれた なまえ だったの かも。",
};

// やり直し（HP0で倒れて街へ戻る）を 世界が薄く覚えている＝周回のメタ語り（責めない・断定しない）。
const RETRY_LINES = ["……まえも、ここで たおれた きが する。", "この夜、もう いちど。", "（また、夜だ。）"];
...
const TUTORIAL = {
  ...
  surprise: "……あれ。きえ ない。",                       // ぶつけても消えない＝“倒すゲームじゃない”の気づき（予想の裏切り）
  coachHit: "ぶつけても、この子は きえない。【きいてみる】で こえを きいてみよう。", // 2回目以降のコーチ
  listen:   "……おいてかないで",                          // きいてみる で見える本音
  reach:    "くろまるは、すこしだけ 近づいた。",           // 手をのばす
  // ※hit（消えた）は v0.9 で未使用：第一夜の くろまる は ぶつけても倒れない（核へ誘導する前フリにする）。
  hit:      "くろまるは、もう いない。",
};
...
const FRIEND_TOWN_LINES = {
  ...
  mizu:  ["ここの みずは、しずか。", "うつるのは、きみと ぼく だけ。"],
  eki:   ["でんしゃ、もう まってない。", "ここが、えき でも いいや。"],
  oku:   ["なまえ、まだ おもいだせない。でも、きみは おぼえてる。", "よんで くれて、ありがとう。"],
};
...
const FIELD_WORD_POOL = ["gomen", "kowakunai", "arigatou", "shizuka", "wakaru", "pien", "soudane", "tsuyotsuyo", "mata", "mizuoto"];
...
const FIELD_FRIEND_POOL = ["kuro", "fura", "usagi", "hoshi", "mori", "mizu", "eki", "oku"];

// 出来事のない歩での「足元の情景」。無音の単調さを避け、夜の道の気配を出す。
//   ぽちが 目盛りを跨ぐたびに 順に切り替える（歩くたびに景色が変わる手触り）。
//   ※v0.12：場所が決まらない夜（第一夜など）の フォールバック。場所つきは FIELD_PLACES。
const FIELD_SCENERY = [
  "おちば が かさり。",
  "とおくに、ちいさな 灯り。",
  "しずかな 夜。むねが すこし さわぐ。",
...
// ── 夜の道の「場所」（v0.12 世界拡張・Toby）──
//   均質な「夜の道」を、地名のある場所へ。夜ごとに ひとつ選ばれ、足元の景色が その場所になる。
//   name＝そっと出す地名 ／ scenery＝歩くたびの情景 ／ friend＝その場所で出会いやすい友（任意）。
//   ※第一夜（チュートリアル）は場所なし（特別さを後で効かせる）。説明はしない＝歩くと景色が変わる、それだけ。
const FIELD_PLACES = {
  mizuumi: { name: "みずうみ の ほとり", friend: "mizu", scenery: [
    "みなもに、つきが ふたつ。", "みずおとが、なにか いいかけて やめた。",
    "あしもとを、さざなみが なでる。", "とおくで、みずが はねた。だれ？",
  ] },
  eki: { name: "こわれた 駅", friend: "eki", scenery: [
    "とまらない でんしゃの おと。", "ホームに、だれかの くつが ひとつ。",
    "とけいは、3じ12ふんで とまってる。", "アナウンスが、ざらりと かすれる。",
  ] },
  mori: { name: "もりの おく", friend: "oku", scenery: [
    "はっぱが、こえを すいこむ。", "おくから、だれかが よんだ きがした。",
    "き の あいだに、しろい いき。", "ふみしめた えだが、ぱきり。",
  ] },
};
// 群れの おく（さいごの夜・ぬしさま戦の手前の専用区画）。倒した子の名残だけが落ちている場所。
const FIELD_OKU = { name: "群れの おく", scenery: [
  "ここは、しずかすぎる。", "あしもとに、きいた ことの ある こえ。",
  "だれかが、ここを とおった。なんども。", "おくに、おおきな けはい。",
] };
