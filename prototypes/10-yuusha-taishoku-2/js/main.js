...
const Game = (() => {
  ...
  function onInteract(thing) {
    ...
    if (thing.kind === 'cabinet') return openDocs();   // §8：資料棚
    if (thing.kind === 'prop') return lookProp(thing); // §9/§10：城の小物
    unlockField();
  }
  ...
  function openDocs() {
    phase = 'dialogue';
    UI.showDocs(DATA.docCards(), () => backToField());
  }
  /* §9/§10：城の小物・部屋の変化を調べる（短い一言） */
  function lookProp(prop) {
    phase = 'dialogue';
    const lines = (prop.look ? prop.look() : [{ who: 'narr', text: '……特に、何もない。' }]);
    SFX.play('page2');
    UI.runDialogue(lines, () => unlockField());
  }

  /* ベルゼ：状況確認 ＋ 次の週へ（＋§4 怒りの誘惑） */
  function talkBelze() {
    phase = 'dialogue';
    UI.runDialogue(DATA.belzeHint(), belzePrompt);
  }
  function belzePrompt() {
    phase = 'dialogue';
    ...
    UI.runDialogue([{ who: 'belze', text: prompt }], null, {
      choices: [
        ...
        { id: 'anger', label: '⚔ つい、王国に手を出したくなる', hint: '——魔王なのに' },
        { id: 'stay', label: '× まだ城を歩く' },
      ],
      onChoice: (ch) => {
        if (ch.id === 'next') advanceWeek();
        else if (ch.id === 'anger') openAnger(belzePrompt);
        else unlockField();
      },
    });
  }
  // §4：怒りの誘惑。攻撃系を選ぶと和平の腕輪が止め、「魔王は我慢した」。押すほどベルゼの台詞が村の話になる。
  function openAnger(onBack) {
    phase = 'dialogue';
    UI.runDialogue([DATA.anger.intro], null, {
      choices: DATA.anger.options.map(o => ({ id: o.id, label: o.label })),
      onChoice: (ch) => {
        const opt = DATA.anger.options.find(o => o.id === ch.id);
        if (!opt || opt.endure) { onBack(); return; }
        if (opt.sfx) SFX.play(opt.sfx);
        // 和平の腕輪のクローズアップを背景に（なぜ殴れないかを画で）
        UI.show('scene'); UI.setSceneDraw((c, tt) => Sprites.scene(c, 'bracelet', tt)); UI.sceneCaption('— 和平の腕輪 —');
        State.angerCount++;
        UI.runDialogue(opt.lines.concat(DATA.anger.belzeAfter(State.angerCount)), () => {
          UI.toast('魔王は我慢した');
          UI.show('field'); UI.renderHUD();
          onBack();
        });
  ...
  function confirmPolicy(policy, thing) {
    ...
    UI.runDialogue(confirm, null, {
      ...
      // §1：touch を持つ施策は「最後のひと手」を選ばせてから実行する
      onChoice: (ch) => { if (ch.id === 'go') { if (policy.touch) presentTouch(policy, thing); else execPolicy(policy, null); } else openDeptPolicies(thing); },
    });
  }
  // §1/§6：最後のひと手——「どう届けるか／どう見せるか」を選ぶ
  function presentTouch(policy, thing) {
    phase = 'dialogue';
    const tc = policy.touch;
    if (tc.scene) { UI.show('scene'); UI.setSceneDraw((c, tt) => Sprites.scene(c, tc.scene, tt)); UI.sceneCaption('— 最後の、ひと手 —'); }
    UI.runDialogue(tc.prompt.slice(), null, {
      choices: tc.choices.map(o => ({ id: o.id, label: o.label, hint: o.hint })),
      onChoice: (ch) => { const choice = tc.choices.find(o => o.id === ch.id); execPolicy(policy, choice); },
    });
  }
  ...
  function collectDocs(d) { return !d ? [] : (Array.isArray(d) ? d : [d]); }
  function execPolicy(policy, touchChoice) {
    ...
    outcome.apply();
    // §1：最後のひと手の上乗せ（やさしい/したたか等の小さな分岐）。演出・会話・資料も差し替える
    let touchTalk = [], touchNote = null, sceneKey = outcome.scene, capText = outcome.cap;
    if (touchChoice) {
      State.setTouch(policy.id, touchChoice.id);
      if (touchChoice.apply) touchChoice.apply();
      if (touchChoice.talk) touchTalk = touchChoice.talk.slice();
      if (touchChoice.note) touchNote = touchChoice.note;
      if (touchChoice.scene) sceneKey = touchChoice.scene;
      if (touchChoice.cap) capText = touchChoice.cap;
    }
    State.lastAction = policy.id;
    State.lastResult = policy.outcomes[resKey] ? resKey : firstKey;
    lastOutcome = outcome;
    // §8：資料棚へ書類が入る（outcome / touch の両方から集める）
    const gained = collectDocs(outcome.doc).concat(collectDocs(touchChoice && touchChoice.doc));
    gained.forEach(id => State.addDoc(id));
    ...
    setDistance();
    // §2：魔王の承認印（ドンッ）→ それから結果の演出へ
    UI.stamp(policy.stamp || '承認', () => {
      if (gained.length) UI.toast('資料棚に〈' + gained.map(id => DATA.documents[id].label).join('・') + '〉が増えた');
      phase = 'result';
      UI.show('scene'); UI.setSceneDraw((c, tt) => Sprites.scene(c, sceneKey, tt));
      UI.sceneCaption(capText + '\n（タップで つづく）');
      SFX.play(State.lastResult === 'fail' ? 'cancel' : 'effect');
      resultTap = () => { resultTap = null; observeAfterPolicy(outcome, touchTalk, touchNote); };
    });
  }
  // §7：現在地描写 → 勇者会話 → 状態変化通知 → 次の一手ヒント
  function observeAfterPolicy(outcome, touchTalk, touchNote) {
    phase = 'observe';
    ...
    if (touchTalk && touchTalk.length) lines = lines.concat(touchTalk); // §1：最後のひと手の余韻
    if (outcome.notify) lines.push({ who: 'narr', text: '【 ' + outcome.notify + ' 】' });
    if (touchNote) lines.push({ who: 'narr', text: '【 ' + touchNote + ' 】' });
    // §7：残留する癖（あれば、たまに混ぜる）
    const res = DATA.residual();
    if (res.length) lines = lines.concat(res[State.turn % res.length]);
  ...
  function advanceWeek() {
    ...
    const mood = DATA.mood(nextTurn); // §11：今週の空気
    UI.curtain(`第${nextTurn}週　／　「${mood ? mood.title : ''}」`, () => {
      State.turn++;
      ...
      else backToField(mood ? `今週：${mood.air}　${mood.lean}` : '新しい週。各部署で施策を1つ。');
    });
  }
  ...
  function bindInput() {
    document.addEventListener('keydown', (e) => {
      ...
      if (UI.docsOpen()) { if (k === 'Escape' || k === ' ' || k === 'Enter') UI.closeDocs(); return; } // 資料棚もキー操作で閉じられる（キーボード専用の詰み防止）
  ...
  function handleDir(dir) {
    ...
    if (UI.docsOpen()) { if (dir === 'act') UI.closeDocs(); return; }  // ●で資料棚を閉じる
    if (UI.newsOpen()) { if (dir === 'act') UI.closeNews(); return; }  // ●でニュースを閉じる
