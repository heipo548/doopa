...
const UI = (() => {
  ...
  let stampCtx = null;        // §2：承認印の朱を描く canvas
  ...
  function init() {
    ['view-title', 'view-field', 'view-scene', 'btn-start', 'btn-continue',
      ...
      'ov-stamp', 'stamp-canvas', 'stamp-label',
      'ov-docs', 'docs-list', 'docs-close', 'docs-empty',
      'toast', 'curtain', 'curtain-text', 'scene-canvas', 'scene-caption']
      .forEach(id => { E[id] = $(id); });
    endingCtx = E['ending-canvas'].getContext('2d');
    stampCtx = E['stamp-canvas'].getContext('2d');
    ...
    E['docs-close'].addEventListener('click', () => closeDocs());
  ...
  function openPolicy(dept, list, onPick, onClose) {
    ...
    list.forEach(item => {
      ...
      const fearWho = pol.fear ? ((DATA.chars[pol.fear.who] || {}).name || '') : '';
      b.innerHTML =
        `<div class="pi-title">${pol.title}` +
        ...
        (item.locked ? `<div class="pi-desc">🔒 ${item.lockMsg || 'まだ解放されていない'}</div>`
          : `<div class="pi-desc">${pol.desc}</div>` +
            // §3：効きやすい状況 / 危険な状況（数値は出さず“怖さ”を言葉で）
            ((pol.when && pol.when.good) ? `<div class="pi-eff">◎ 効きやすい：${pol.when.good}</div>`
              : (pol.preview && pol.preview.effective ? `<div class="pi-eff">◎ 効きそう：${pol.preview.effective}</div>` : '')) +
            ((pol.when && pol.when.bad) ? `<div class="pi-risk">⚠ 危険：${pol.when.bad}</div>`
              : (pol.risk ? `<div class="pi-risk">⚠ ${pol.risk}</div>` : '')) +
            (pol.fear ? `<div class="pi-fear">「${pol.fear.text}」<span class="pi-fear-who">— ${fearWho}</span></div>` : '') +
            (pol.touch ? `<div class="pi-tag">✦ 最後のひと手：実行時に“どう届けるか”を選ぶ</div>` : '') +
            (pol.tag ? `<div class="pi-tag">✦ ${pol.tag}</div>` : ''));
  ...
  /* ── §2：魔王の承認印「ドンッ」。施策決定の瞬間、朱の角印が押される ── */
  function stamp(label, onDone) {
    Sprites.seal(stampCtx, label || '承認');
    E['stamp-label'].textContent = label || '承認';
    const inner = E['ov-stamp'].querySelector('.stamp-inner');
    E['ov-stamp'].classList.remove('hidden');
    // アニメを毎回頭から（連続でもきちんと“押す”）
    if (inner) { inner.style.animation = 'none'; void inner.offsetWidth; inner.style.animation = ''; }
    setTimeout(() => SFX.play('stamp'), 200); // 振り下ろしと音を合わせる
    setTimeout(() => { E['ov-stamp'].classList.add('hidden'); if (onDone) onDone(); }, 1050);
  }
  function stampShown() { return E['ov-stamp'] && !E['ov-stamp'].classList.contains('hidden'); }

  /* ── §8：資料棚（集めた書類をカードで読む） ── */
  let docsOnClose = null;
  function showDocs(cards, onClose) {
    docsOnClose = onClose || null;
    const box = E['docs-list']; box.innerHTML = '';
    if (!cards || !cards.length) { E['docs-empty'].classList.remove('hidden'); }
    else {
      E['docs-empty'].classList.add('hidden');
      cards.forEach(d => {
        const el = document.createElement('div'); el.className = 'doc-card';
        el.innerHTML = `<div class="doc-label">${d.icon || '📄'} ${d.label}</div>` +
          `<div class="doc-stat">${d.card.stat}</div>` +
          (d.card.note ? `<div class="doc-note">${d.card.note}</div>` : '');
        box.appendChild(el);
      });
    }
    E['ov-docs'].classList.remove('hidden'); SFX.play('page2');
  }
  function closeDocs() { E['ov-docs'].classList.add('hidden'); const cb = docsOnClose; docsOnClose = null; SFX.play('cancel'); if (cb) cb(); }
  function docsOpen() { return !E['ov-docs'].classList.contains('hidden'); }

  /* ── 何らかのオーバーレイが開いているか（フィールド入力を止める判定に使う） ── */
  function anyOverlay() { return dia.active || policyOpen() || helpOpen() || newsOpen() || endingActive || docsOpen() || stampShown(); }
  ...
  return {
    ...
    stamp, stampShown, showDocs, closeDocs, docsOpen,
