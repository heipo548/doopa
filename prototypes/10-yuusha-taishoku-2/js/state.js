...
const State = {
  ...
  tutorialDone: false,
  angerCount: 0,       // §4：玉座で「怒りの誘惑」を押した回数（増えるとベルゼの台詞が変わる）
  documents: {},       // §8：資料棚にたまった書類 { clinic_record: true, ... }
  touchChoices: {},    // §1：施策ごとに選んだ「最後のひと手」 { clinic: 'flag_up', ... }
  ...
  /* §8：資料棚 — 施策の結果として“証拠/書類”がたまり、反論・最終面談で使える */
  addDoc(id) { if (id) this.documents[id] = true; },
  hasDoc(id) { return !!this.documents[id]; },
  docCount() { return Object.keys(this.documents).length; },
  /* §1：最後のひと手の記録（観察・残留演出で参照する） */
  setTouch(policyId, choiceId) { this.touchChoices[policyId] = choiceId; },
  touch(policyId) { return this.touchChoices[policyId] || null; },
  ...
  // proto#10 は#09と別キー（同じブラウザで両方遊んでもセーブが混ざらない）
  KEY: 'doopa10_save_v1',
  save() {
    try {
      const data = {
        ...
        angerCount: this.angerCount, documents: this.documents, touchChoices: this.touchChoices,
      };
  ...
  load() {
    try {
      ...
      Object.assign(this, {
        ...
        angerCount: d.angerCount || 0, documents: d.documents || {}, touchChoices: d.touchChoices || {},
      });
