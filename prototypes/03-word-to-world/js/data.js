...
const ENEMIES = {
  ...
  kojika: {
    ...
    lines: {
      onHarsh: ["ひゃっ……！", "やめてよぉ……"],
      // onKind は「警戒→ほどける→うれしい」の順（思いやり進捗で段階出し分け）。
      onKind:  ["……ほんと？", "やさし……い？", "……うれし、い。"],
  ...
  kakashi: {
    ...
    lines: {
      ...
      onKind:  ["……き、いて くれるの。", "ぼく、にも？", "……ありがとう。きいて くれて。"],
  ...
  villageChief: {
    ...
    lines: {
      ...
      defeatHarsh: "村長は ことばを なくし、よろめいて うずくまった。「もう……いい。……この先の 洞窟に、わしより 怒れる ものが いる。気を つけろ」",
      defeatKind:  "村長は ふっと 肩の力を ぬいた。「……ありがとう。ひさしぶりに、そう 言われた。……洞窟の おくの あの子にも、その ことばが 届くと いいが」",
    },
  },
...
const AREAS = {
  mura: {
    ...
    // 入場時に足元へ大きく出す“目的の一文”＋次の小目標（説明ゼロ即操作のまま、最初の30秒の迷いを消す／堀井FB）。
    intro: "ことばを おぼえて、村を あるこう。",
    introSub: "だれかに 会えば、ことばが もらえる。",
    nodes: [
  ...
  dokutsu: {
    ...
    intro: "ことばに 傷ついた ものが ねむる 洞窟。",
    introSub: "さきへ すすめば、おくに だれかが いる。",
    nodes: [
...
const META = {
  ...
  // 締めを結末トーン別に出し分ける（メタを“採点”でなく“返礼”に／Toby・堀井FB）。
  //   warm＝温かく返す／gray＝両方を抱える／cruel＝突き放さず「責めない・見ていた・やり直せる」。
  closeByTone: {
    warm: [
      "きみは だれも 言い負かさなかった。",
      "きみが いたから、ことばは 世界に のこった。",
      "なまえを もった きみは、もう 型番じゃ ない。きみの 名は、L。",
      "……ありがとう。またね、きっと どこかで。",
    ],
    gray: [
      "きみの 中には、言い負かした ことも、寄りそった ことも のこっている。",
      "どちらも、きみだ。",
      "やさしい 道、かたい 道、その あいだ。……まだ 見て いない 朝が、ある。",
      "なまえを もった きみは、もう 型番じゃ ない。またね、L。",
    ],
    cruel: [
      "きみは ぜんぶ 言い負かした。せかいは しずかに なった。",
      "……でも、せめて いる わけじゃ ない。ただ、見て いたよ。",
      "ことばは、まだ きえて いない。もういちど 歩けば、ちがう ことばも 言える。",
      "なまえを もった きみは、もう 型番じゃ ない。またね、L。",
    ],
  },
};
