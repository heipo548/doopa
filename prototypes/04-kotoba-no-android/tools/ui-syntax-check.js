...
var test = [
  ...
  // ── 回帰：戦闘コマンドが“次も”選べるか（詰まり防止）──
  "newGame(); startBattle('kadokko'); app.battleBusy=false;",
  "onCard('urusai');", // 一手目：busy=true（敵ターン待ち）。disabled属性は付けない設計＝固まらない
  "var __cb1=document.getElementById('command-bar').innerHTML;",
  "__o.push('一手目後: busy='+app.battleBusy+'（true期待＝連打ガード）/ disabled属性なし='+(__cb1.indexOf('disabled')<0)+'（true期待＝二度と固まらない）');",
  "resolveEnemyTurn();", // 敵ターン解決：busy解除＋再描画
  "__o.push('敵ターン後: busy='+app.battleBusy+'（false期待）/ battle継続='+(!!game.battle));",
  "var __b=app.battleBusy; onCard('urusai'); __o.push('二手目 受理='+(__b===false && app.battleBusy===true)+'（true期待＝連続選択OK）');",
  "resolveEnemyTurn(); __o.push('三手目前 busy='+app.battleBusy+'（false=詰まらない）');",
  // 残り画面
