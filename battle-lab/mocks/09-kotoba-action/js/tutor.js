 ...
 *  Tutor.init({
 *    objective:'目標：…',
 *    terms:[{term:'こころ',desc:'Lの元気。0で失敗。'}, …],
 *    steps:[{title:'…',body:'…',highlight:'#cmd-area button'}, …]
 *  });
...
var Tutor = (function () {
  ...
  /* ── ステップ式チュートリアル ── */
  function openSteps() {
    if (!cfg.steps || !cfg.steps.length) return;
    idx = 0; showStep();
  }
  function showStep() {
    var s = cfg.steps[idx];
    ...
    var next = el('button','tutor-next'); next.textContent=(idx>=cfg.steps.length-1)?'はじめる':'次へ';
  ...
  function nextStep() {
    if (idx >= cfg.steps.length - 1) { closeSteps(); return; }
    idx++; showStep();
  }
  function closeSteps() {
    var ov = $('tutor-overlay'); if (ov) ov.classList.remove('show');
    clearHighlight();
    // チュートリアルを閉じたら、ゲーム側の「おすすめ手」ハイライトを出し直す
    // （まだ1手も打っていなければ recommend() が再点灯する）
  ...
  function highlight(selector) {
    try {
      ...
      for (var i=0;i<nodes.length;i++) nodes[i].classList.add('tutor-hi');
    } catch (e) {}
  }
  function clearHighlight() {
    try {
      var nodes = document.querySelectorAll('.tutor-hi');
      for (var i=0;i<nodes.length;i++) nodes[i].classList.remove('tutor-hi');
    } catch (e) {}
  }
